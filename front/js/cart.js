...
async function displayProduct(panier) {
  ...
  for (product of panier) {
    ...
    const divImg = document.createElement("div"); // création div img
    ...
    const divContent = document.createElement("div"); // création d'une div content
    divContent.classList.add("cart__item__content");
    articleNode.appendChild(divContent);

    const divDescription = document.createElement("div"); // création div description
    ...
    const divSettings = document.createElement("div"); // création div settings
    divSettings.classList.add("cart__item__content__settings");
    divContent.appendChild(divSettings);
    const divQuantity  = document.createElement("div") // création div quantity
    divSettings.appendChild(divQuantity)
    const divPrice = document.createElement("div"); // création div price
    divPrice.classList.add("cart__item__content__settings");
    divQuantity.appendChild(divPrice)
    const pricep = document.createElement("p");
    pricep.innerText = `Prix à l'unité : ${product.info.price}€`;
    ...
    const divDelete = document.createElement("div"); // création div delete
    divDelete.classList.add("cart__item__content__settings__delete");
    divSettings.appendChild(divDelete);
    const deletep = document.createElement("p");
    deletep.classList.add("deleteItem");
    deletep.setAttribute("title", "Pour supprimer, cliquez ici");
...
/* deletep.addEventListener("mouseover",);
      alert("Pour supprimer le produit clicker ici"); */
...
function listenQuantity() {
  ...
  quantityArray.forEach(function (elm) {
    elm.addEventListener("input", function (e) {
      ...
      let articleColor = elm.closest("article").getAttribute("data-color");
      let index = panier.findIndex(
        (produit) => produit.id === articleId && produit.color === articleColor
      );
      if (e.target.value <= 0 || e.target.value > 100) {
        e.target.value = panier[index].quantity;
        return window.alert(
          "la quantité ne peut être inférieure à 1, et supérieure à 100. Cliquez sur supprimer si vous voulez le retirer du panier, ou modifiez la quantité."
        );
      } else {
...
const alphaNumberRegex =
  /^[0-9a-zA-Zçñàéèêëïîôüù][0-9a-zA-Zçñàéèêëïîôüù '-\.]{2,}$/;
const emailRegex =
...
function checkInput() {
  // fonction de vérification des inputs du formulaire lors de la saisie des infos par l'utilisateur
...
function checkForm() { 
  // fonction qui vérifie que tous les input sont ok
  if (
    ...
    alert("Veuilez remplir tout les champs avant de commander")
    return false;
  }
}
...
function listenOrder() {
  ...
  order.addEventListener("click", function (event) {
    ...
    if (inputValid == true) {
      ...
      const order = {
        // création de l'objet contenant les infos du client et l'id des produits commandés
        contact: contact,
        products: produit,
      };

      const options = {
        // "définition" de la réquête
        method: "POST",
      ...
      fetch("http://localhost:3000/api/products/order", options) // Envoie de la requête
        ...
        .then(function (resData) {
          window.location.href =
            "../html/confirmation.html?orderId=" + resData.orderId;
          localStorage.clear();
        })
        .catch(function (err) {
...
function main() {
  // fonction principale qui appelle des fonctions pour qu'elles s'exécutent
  let panier = JSON.parse(localStorage.getItem("cart"));
